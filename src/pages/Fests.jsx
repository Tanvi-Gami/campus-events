    ...
    <div className="min-h-screen bg-[#020617] text-white font-['Inter']">
      ...
      <main className="pt-28 pb-12 px-6 max-w-7xl mx-auto space-y-8">
        ...
        <div>
          ...
          <p className="text-[#CBD5E1]/70">
            Organizer-managed fests with nested event pages and live registrations.
          </p>
          {message && (
            <p className="mt-2 text-sm text-[#22C55E]">{message}</p>
          )}
        </div>
        ...
        {role === "organizer" && (
          <section className="rounded-2xl border border-white/10 bg-[#0F172A]/70 p-6">
            <h2 className="text-xl font-bold mb-3">
              {editingId ? "Edit Fest" : "Create Fest"}
            </h2>
            ...
            <form onSubmit={submitFest} className="grid md:grid-cols-2 gap-4">
              ...
              <input required
                className="p-3 rounded-lg bg-white/5 border border-white/10"
                placeholder="Registrations close on"
                value={form.registrationsCloseOn}
                onChange={(e) =>
                  setForm({
                    ...form,
                    registrationsCloseOn: e.target.value,
                  })
                }
              />
              ...
              <textarea
                rows={3}
                className="md:col-span-2 p-3 rounded-lg bg-white/5 border border-white/10"
              ...
              <div className="md:col-span-2 flex gap-3">
                <button className="px-5 py-2 rounded-lg bg-[#A855F7] font-bold">
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          {fests.map((fest) => (
            ...
            >
              <p className="text-xs uppercase tracking-widest text-[#06B6D4] font-bold">
                {fest.theme}
              </p>

              <h2 className="text-2xl font-black mt-1 mb-2">
                {fest.name}
              </h2>

              <p className="text-sm text-[#CBD5E1]/70 mb-4">
                {fest.overview}
              </p>
              ...
              <p className="text-sm mb-4">
                ✅ {fest.registrationsOpen ? "Open" : "Closed"} · Closes on{" "}
                {fest.registrationsCloseOn}
              </p>
                  ...
                  <button
                    onClick={() => {
                      ...
                      setForm({
                        ...
                        venue: fest.venue || "",
                        registrationsOpen:
                          Boolean(fest.registrationsOpen),
                        registrationsCloseOn:
                          fest.registrationsCloseOn || "",
